    ...
    if (loading) return (
        <div className="flex h-screen items-center justify-center bg-[#FDFDFD]">
            <div className="relative flex flex-col items-center gap-4">
                <div className="absolute inset-0 rounded-full bg-[#00AEEF]/20 blur-3xl animate-pulse scale-150"></div>
                <Loader2 className="animate-spin text-[#00AEEF] relative z-10" size={52} strokeWidth={1.5} />
                <p className="text-[#00AEEF] text-xs font-mono tracking-[0.4em] uppercase relative z-10">Loading</p>
            </div>
        </div>
    );
        ...
        <div className="p-6 md:p-10 bg-[#FDFDFD] min-h-screen font-[Inter] relative overflow-x-hidden">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <header className="mb-14 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <p className="text-[#00AEEF] text-[10px] font-mono uppercase tracking-[0.4em] mb-3 flex items-center gap-2">
                            <span className="inline-block w-6 h-px bg-[#00AEEF]"></span> Admin Management Portal
                        </p>
                        <h2 className="text-5xl md:text-6xl font-[Playfair_Display] font-bold leading-none tracking-tighter text-gray-900">
                            Financial <span className="text-[#00AEEF]">Hub</span>
                        </h2>
                    </div>
                    <button onClick={() => navigate('/admin/payment-report')} 
                        className="group relative flex items-center gap-3 overflow-hidden bg-[#00AEEF] text-white px-7 py-3.5 rounded-full font-bold shadow-lg hover:shadow-xl hover:scale-[1.03] transition-all duration-500 text-[11px] uppercase tracking-widest">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12">
                    ...
                    <div className="group relative overflow-hidden bg-white border border-[#00AEEF]/20 p-7 rounded-3xl hover:border-[#00AEEF]/50 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl shadow-sm">
                        <div className="flex items-center gap-5 relative z-10">
                            <div className="bg-[#00AEEF]/10 border border-[#00AEEF]/20 text-[#00AEEF] p-4 rounded-2xl shadow-sm group-hover:scale-110 transition-transform duration-300">
                                <Wallet size={28} strokeWidth={1.5} />
                            </div>
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#00AEEF]/70 mb-1.5">Crypto Volume</p>
                                <h4 className="text-3xl font-black text-gray-800 leading-none tracking-tight">
                                    <span className="text-[#00AEEF]">{stats.cryptoCount}</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-12">
                    <div className="lg:col-span-2 bg-white border border-gray-100 p-7 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-500">
                        <div className="flex items-center mb-7 gap-3 border-b border-gray-100 pb-4">
                            <div className="p-2 bg-[#00AEEF]/10 rounded-xl border border-[#00AEEF]/20">
                                <TrendingUp className="text-[#00AEEF]" size={16} strokeWidth={2} />
                            </div>
                        ...
                        <div className="h-[280px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={stats.revenueData}>
                                    ...
                                    <Area type="monotone" dataKey="amount" stroke="#00AEEF" fill="url(#colorRev)" strokeWidth={2.5} />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    <div className="bg-white border border-gray-100 p-7 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-500">
                        <div className="flex items-center mb-7 gap-3 border-b border-gray-100 pb-4">
                            <div className="p-2 bg-[#00AEEF]/10 rounded-xl border border-[#00AEEF]/20">
                                <PieIcon className="text-[#00AEEF]" size={16} strokeWidth={2} />
                            </div>
                ...
                <div className="flex justify-end mb-5">
                    <div className="flex items-center gap-3 bg-white border border-gray-200 p-1.5 pl-5 pr-2 rounded-full shadow-sm">
                        <Filter size={14} className="text-[#00AEEF]" />
                        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                            className="bg-transparent border-none text-[10px] font-black uppercase tracking-widest focus:ring-0 cursor-pointer text-gray-500 border-r border-gray-200 pr-4 py-2 hover:text-[#00AEEF] transition-colors outline-none">
                        ...
                        <select value={filterMethod} onChange={(e) => setFilterMethod(e.target.value)}
                            className="bg-transparent border-none text-[10px] font-black uppercase tracking-widest focus:ring-0 cursor-pointer text-gray-500 pl-2 pr-4 py-2 hover:text-[#00AEEF] transition-colors outline-none">
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {filteredPayments.map((payment) => (
                                    <tr key={payment._id} className="hover:bg-[#00AEEF]/5 transition-all duration-500 group">
                                        <td className="px-8 py-5">
                                            <div className="font-black text-gray-800 text-sm uppercase tracking-tight">{payment.userId?.firstName || "Guest"}</div>
                                            <div className="text-[9px] text-[#00AEEF] font-black uppercase tracking-widest mt-1">{payment.paymentMethod}</div>
                                        </td>
                                        <td className="px-8 py-5">
                                        ...
                                        <td className="px-8 py-5 text-center">
                                            <button onClick={() => { setSelectedPayment(payment); setIsModalOpen(true); }} 
                                                className="p-3 bg-gray-50 border border-gray-200 text-gray-400 rounded-xl hover:bg-[#00AEEF] hover:text-white hover:border-[#00AEEF] hover:shadow-lg transition-all duration-500 group-hover:scale-105">
                                                <Eye size={16} />
                                            </button>
                                        </td>
                                    </tr>
            ...
            {isModalOpen && selectedPayment && (
                <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-md flex items-center justify-center p-4 z-50">
                    <div className="bg-white border border-gray-200 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl">
                        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gradient-to-r from-[#00AEEF]/10 to-white">
                            <div>
                                ...
                                <p className="text-[9px] text-[#00AEEF] font-mono tracking-[0.3em] uppercase mt-0.5">Admin Review Panel</p>
                            </div>
                        ...
                        <div className="p-8">
                            <div className="grid grid-cols-2 gap-4 mb-7">
                                {selectedPayment.paymentMethod === 'crypto' ? (
                                    <div className="p-5 bg-[#00AEEF]/5 rounded-2xl border border-[#00AEEF]/20 col-span-2">
                                        <span className="text-[9px] text-[#00AEEF] font-black uppercase tracking-widest block mb-2">TxHash Identifier</span>
                                        <p className="text-[11px] font-mono break-all text-gray-600 leading-relaxed bg-white p-3 rounded-xl border border-[#00AEEF]/10">{selectedPayment.transactionId}</p>
                                    </div>
                                ) : (
                                    <>
                                        <div className="p-5 bg-[#00AEEF]/5 rounded-2xl border border-[#00AEEF]/10">
                                            <span className="text-[9px] text-[#00AEEF] font-black uppercase tracking-widest block mb-2">Bank</span>
                                            <p className="text-gray-800 font-black text-xs uppercase">{selectedPayment.paymentDetails?.bankName || 'N/A'}</p>
                                        </div>
                                        <div className="p-5 bg-[#00AEEF]/5 rounded-2xl border border-[#00AEEF]/10">
                                            <span className="text-[9px] text-[#00AEEF] font-black uppercase tracking-widest block mb-2">Branch</span>
                            ...
                            <div className="mb-7 flex justify-center">
                                {selectedPayment.paymentMethod === 'bank_transfer' ? (
                                    ...
                                    <div className="h-44 w-full bg-gradient-to-br from-[#00AEEF]/5 to-[#00AEEF]/10 rounded-2xl flex flex-col items-center justify-center border border-[#00AEEF]/20 shadow-sm">
                                        <Wallet size={44} className="mb-4 text-[#00AEEF] animate-pulse"/>
                                        <a href={`https://blockchain.info/tx/${selectedPayment.transactionId}`} target="_blank" className="bg-[#00AEEF] text-white text-[9px] font-black px-6 py-2.5 rounded-full hover:shadow-lg transition-all uppercase tracking-widest hover:scale-105">Verify On Explorer</a>
                                    </div>
                                )}
                            </div>
