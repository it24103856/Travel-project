...
const AdminCreateDestination = () => {
  ...
  const handleChange = (e) => {
    ...
    // Province එක වෙනස් කළහොත් District සහ City Reset කරන්න
    if (name === "province") {
      setFormData({ ...formData, province: value, district: "", city: "" });
    } 
    // District එක වෙනස් කළහොත් City Reset කරන්න
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/destinations/create`, formData, {
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-6">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-2xl font-black text-gray-900 uppercase italic">Add Destination</h1>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                <Type size={16} className="text-blue-500" /> Name
              </label>
              <input type="text" name="name" required className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl outline-none" placeholder="Destination Name" onChange={handleChange} />
            </div>
            ...
            <div>
              ...
              <textarea name="description" rows="4" required className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl outline-none" placeholder="Description..." onChange={handleChange}></textarea>
            </div>

            {/* Image Upload */}
            <div>
              <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                <ImageIcon size={16} className="text-blue-500" /> Images
              </label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <label className="cursor-pointer border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center h-32 hover:bg-gray-50 transition-all group">
                  ...
                  {uploading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div> : <UploadCloud className="text-gray-400 group-hover:text-blue-500" />}
                </label>
            ...
            <button type="submit" disabled={loading || uploading} className="w-full py-5 rounded-2xl font-black uppercase text-white bg-gray-900 hover:bg-blue-600 transition-all disabled:bg-gray-300">
