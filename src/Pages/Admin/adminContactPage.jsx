      ...
      <div className="w-full max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="mb-10 text-center md:text-left">
          <h1 className="text-4xl font-[Playfair_Display] font-bold text-gray-900">
            Contact <span className="text-[#6366F1]">Management</span>
          </h1>
          <p className="text-gray-500 mt-2 text-lg font-[Inter]">Configure the official contact details for your travel platform.</p>
          <div className="w-24 h-1.5 bg-[#6366F1] mt-4 rounded-full mx-auto md:mx-0"></div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm hover:shadow-2xl overflow-hidden border border-gray-100 transition-all duration-500">
          <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-8 text-white flex justify-between items-center">
            ...
            <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-lg">
              {isEditing ? <Pencil className="text-[#6366F1] w-6 h-6 animate-pulse" /> : <User className="text-[#6366F1] w-6 h-6" />}
            </div>
          </div>
          ...
          {!isEditing && contact ? (
            <div className="p-10">
              ...
              <div className="flex gap-4 mt-12">
                <button onClick={() => setIsEditing(true)} className="flex-1 bg-[#6366F1] hover:bg-[#4F46E5] text-white font-bold py-4 rounded-full shadow-lg transition-all duration-500 flex items-center justify-center gap-3 active:scale-95 uppercase tracking-widest text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea name="address" value={formData.address} onChange={handleChange} required rows="3"
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#6366F1]/10 focus:border-[#6366F1] focus:bg-white outline-none transition-all duration-500 font-[Inter]"
                    placeholder="Street, City, Country"></textarea>
                </div>
              </div>
              <div className="flex gap-4 pt-4">
                <button type="submit" disabled={loading} className="flex-1 bg-[#6366F1] hover:bg-[#4F46E5] text-white font-bold py-4 rounded-full transition-all duration-500 flex items-center justify-center gap-3 shadow-xl active:scale-95 disabled:bg-gray-400 uppercase tracking-widest text-xs">
...
function InfoBlock({ icon, label, value }) {
  return (
    <div className="flex items-start gap-4 p-5 rounded-2xl bg-gray-50/50 hover:bg-[#6366F1]/5 transition-all duration-500 border border-transparent hover:border-[#6366F1]/10">
      <div className="bg-white p-3.5 rounded-xl text-[#6366F1] shadow-sm border border-gray-100">{icon}</div>
      <div>
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input {...props} required className="w-full pl-12 pr-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#6366F1]/10 focus:border-[#6366F1] focus:bg-white outline-none transition-all duration-500 font-[Inter]" />
      </div>
    </div>
  );
}
