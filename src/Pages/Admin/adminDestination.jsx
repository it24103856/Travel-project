...
const AdminDestinations = () => {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      icon: "warning", showCancelButton: true, confirmButtonColor: "#6366F1", cancelButtonColor: "#6b7280",
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-3xl font-[Playfair_Display] font-bold text-gray-900">
              Manage <span className="text-[#6366F1]">Destinations</span>
            </h1>
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#6366F1] transition-colors" size={18} />
              <input type="text" placeholder="Search name, city or district..." 
                className="pl-12 pr-6 py-3 bg-white border border-gray-200 rounded-full shadow-sm focus:ring-2 focus:ring-[#6366F1]/20 focus:border-[#6366F1] outline-none w-full md:w-64 transition-all duration-500 font-medium text-sm"
                onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <Link to="/admin/destinations/add" className="flex items-center gap-2 bg-[#6366F1] hover:bg-[#4F46E5] text-white px-6 py-3.5 rounded-full font-bold transition-all duration-500 shadow-lg active:scale-95 group uppercase tracking-widest text-xs">
              ...
              <div key={loc._id} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col">
                <div className="relative h-60 overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-4 group-hover:translate-x-0">
                    ...
                    <Link to={`/admin/destinations/edit/${loc._id}`} className="p-3 bg-white text-gray-700 rounded-2xl shadow-lg hover:bg-[#6366F1] hover:text-white transition-all duration-500"><Edit3 size={18} /></Link>
                  </div>
                </div>
                <div className="p-6 flex flex-col flex-1">
                  <div className="mb-4">
                    <h3 className="text-xl font-[Playfair_Display] font-bold text-gray-900 leading-none truncate group-hover:text-[#6366F1] transition-colors">{loc.name}</h3>
                  ...
                  <div className="mt-auto pt-5 border-t border-gray-50 flex justify-between items-center">
                    ...
                    <Link to={`/overview/${loc._id}`} className="text-gray-300 hover:text-[#6366F1] transition-all duration-500 hover:scale-110">
                      <Eye size={20} />
                    </Link>
                  </div>
                </div>
